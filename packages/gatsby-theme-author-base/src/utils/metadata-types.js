...
class MetaType {
    ...
    getProperties() {
        return Object.keys(this.properties).reduce((properties, property) => {
            ...
            if (typeof property === MetaType) {
                const nestedProperties = property.getProperties()
                return [...properties, ...nestedProperties]
            }
            return properties.push({property, content: this.properties[property]})
        }, [])
    }
    getNameAttributes() {
        return Object.keys(this.properties).reduce((names, name) => {
            return this.properties[name] ? names.push({name, content: this.properties[name]}) : names
        }, [])
    }
    addTag(tag) {
    ...
    addProfile({type, profile}) {
        ...
        const profileType = new ProfileType(...profile)
        if (Array.isArray(this.properties[keys[idx]])) {
            this.properties[keys[idx]].push(profileType)
        } else {
            this.properties[keys[idx]] = profileType
        }
    }
}
...
export class FacebookType extends MetaType {
    constructor(data) {
        ...
        this.properties['og:locale:alternate'] = data.alternateLocales.map(locale => new AlternateLocaleType(locale))
...
export class MusicType extends MetaType {
    constructor(subtype, data) {
        ...
        if (subtype === `song`) {
            ...
            this.properties[`music.musician`] = data.musicians.map(musician => this.addProfile(...musician))
...
export class VideoType extends MetaType {
    constructor(subtype, data) {
        ...
        if (subtype === `episode` && data) {
            this.properties['video.actor'] = data.actors.map(actor => new ProfileType(...actor, `actor`))
            this.properties['video.director'] = []
            this.properties['video.writer'] = []
            this.properties['video.duration'] = null
            this.properties['video.release_date'] = null
